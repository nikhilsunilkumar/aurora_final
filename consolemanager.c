 ...
 * @return 		success - if vaild input is received
 ...
 * Revision History:
 * 	- 130121 DA: 	Creation Date
 * 	- 250121 DA:    Added one more validation for change rate
 */
int validate_buffer(param_t buffer_id, uint8_t *data)
{
	uint32_t int_value;
	uint32_t ref_rate;
	char *p=0,temp_value[2];
	...
	switch(buffer_id)
	{
		...
		case CHANGE_RATE_ID:
			memcpy(temp_value,curr_config.refresh_rate,sizeof(curr_config.refresh_rate));
			ref_rate=atoi(temp_value);
			if(int_value<MIN_CHANGE_RATE || int_value>MAX_CHANGE_RATE || *p!=0)
				return FAIL;
			if((int_value % ref_rate)!=0)
				return COMPARE_FAIL;
			break;
...
int validate_executionkey(uint8_t byte)
{
	...
	else if(mode_value == AUTO && byte == ' '){
		return SUCCESS;
	}

	else if(mode_value == MANUAL &&( byte == '>' || byte == '<')){
		return SUCCESS;
	}
	return FAIL;
}
